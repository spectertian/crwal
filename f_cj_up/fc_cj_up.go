...
forStart:
	...
	for _, info := range m.List {
		...
		vod_id := db.SaveAndUpdateFj(&info)
		if vod_id == "" {
			fmt.Println("vod_id为空", info, vod_id)
			return
		}
		db.SaveVodImageFjById(vod_id, info.VodPic, "fj_")
...
func main() {
	...
	url := "https://demo.if101.tv/api.php/provide/vod/at/json?ac=detail&h=25&pg=%v"
...
func SetPageCounts() {
	url := "https://demo.if101.tv/api.php/provide/vod/at/json?ac=detail&h=25"
	fmt.Println(url)
	res, err := http.Get(url)
	if err != nil {
		log.Fatal(err)
