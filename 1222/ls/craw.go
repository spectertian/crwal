...
func main() {
	...
	url := "https://www.tingshuge.com/book/30937.html"
	books := &Book{}
	...
	doc.Find(".listmain dd").Each(func(i int, s *goquery.Selection) {
		...
		chapters := &Chapter{}
		chapters.Url = "https://www.tingshuge.com" + href

		if IsChapterOk(chapters.Url) != "" {
			fmt.Println(re_title, "已抓取")
			return
		}
		...
		if href != "" {
			fmt.Println("进入队列 ", chapters.Title)
			go GetChapterInfo(ch, chapters)
			//go func() {
			//	ch <- *chapters
			//
			//}()
...
func saveContents(ch chan Chapter, sign chan byte) {
		...
		case ss, ok := <-ch:
			if ok {
				...
				if contents != "" {
					...
					fmt.Println("消费队列： ", chapters.Title)
