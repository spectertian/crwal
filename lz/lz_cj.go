...
forStart:
	...
	for _, info := range m.List {
		...
		vod_id := db.SaveAndUpdateLZ(&info)
		if vod_id == "" {
			fmt.Println("vod_id为空", info, vod_id)
			return
		}
		db.SaveVodImageById(vod_id, info.VodPic, "lz_")
...
func main() {
	...
	url := "https://cj.lziapi.com/api.php/provide/vod/at/json?ac=detail&pg=%v"
...
func SetPageCounts() {
	url := "https://cj.lziapi.com/api.php/provide/vod/at/json?ac=detail"
	fmt.Println(url)
	res, err := http.Get(url)
	if err != nil {
		log.Fatal(err)
